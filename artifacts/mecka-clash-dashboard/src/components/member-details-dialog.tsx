    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-[#061827]/70 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-label={`Player card ${name}`} onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <section className="max-h-[90dvh] w-full max-w-[620px] overflow-y-auto rounded-t-3xl border border-card-border bg-card shadow-2xl sm:rounded-3xl">
        ...
        <div className="grid gap-3 p-5 sm:grid-cols-2">
          ...
          <div className="rounded-2xl bg-secondary/60 p-4"><div className="flex items-center gap-2 text-primary"><Gift className="size-4" /><span className="text-[10px] font-bold uppercase tracking-[.12em]">Donations</span></div><p className="mt-2 font-data text-2xl font-bold">{compact(num(member.donations))}</p><p className="mt-1 text-xs text-muted-foreground"> received {compact(num(member.donationsReceived))}</p></div>
