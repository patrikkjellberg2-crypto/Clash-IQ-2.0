...
const HISTORICAL_RECOVERY_TTL_MS = 15 * 60_000;
const historicalRecoveryCache = new Map<
  string,
  { expiresAt: number; promise: Promise<void> }
>();

/**
 * Coalesces concurrent archive recovery and keeps the expensive upstream
 * lookup warm for a short period. Opening several player cards should never
 * trigger the same ClashKing/Clash API recovery work once per player.
 */
async function recoverHistoricalWars(
  ...
  const cacheKey = normalizeClanTag(clanTag);
  const now = Date.now();
  const cached = historicalRecoveryCache.get(cacheKey);

  if (cached && cached.expiresAt > now) {
    return cached.promise;
  }

  const promise = recoverHistoricalWarsUncached(
    cacheKey,
    log,
    maxWars,
  );

  historicalRecoveryCache.set(cacheKey, {
    expiresAt: now + HISTORICAL_RECOVERY_TTL_MS,
    promise,
  });

  // Unexpected failures should be retryable; only successful/best-effort
  // recovery remains warm for the full TTL.
  void promise.catch(() => {
    if (historicalRecoveryCache.get(cacheKey)?.promise === promise) {
      historicalRecoveryCache.delete(cacheKey);
    }
  });
 ...
 */
async function recoverHistoricalWarsUncached(
  clanTag: string,
  log: { warn: (obj: object, message: string) => void },
  maxWars: number,
): Promise<void> {
  ...
  try {
    ...
    const candidates = listItems(warlog.data)
      .slice(0, maxWars)
      .flatMap((listed) => {
        const endTime = String(listed.endTime ?? "").trim();
        const clan =
          listed.clan && typeof listed.clan === "object"
            ? (listed.clan as ClashRecord)
            : null;
        const opponent =
          listed.opponent && typeof listed.opponent === "object"
            ? (listed.opponent as ClashRecord)
            : null;

        if (!endTime || !clan || !opponent) return [];

        const otherTag = normalizeClanTag(String(opponent.tag ?? ""));
        const key = `${requested}__${otherTag}__${endTime}`;
        return seen.has(key) ? [] : [{ endTime }];
      });

    // Limit concurrency so recovery is faster than a fully serial loop while
    // still being courteous to the public upstream API.
    const batchSize = 3;
    for (let offset = 0; offset < candidates.length; offset += batchSize) {
      await Promise.all(
        candidates.slice(offset, offset + batchSize).map(async ({ endTime }) => {
          try {
            ...
            for (const war of listItems(detail.data).slice(0, 1)) {
              await save(war);
            }
          } catch (error) {
            log.warn(
...
async function getActiveClanTag(
  ...
  return normalizeClanTag(selection?.clanTag ?? DEFAULT_CLAN_TAG);
}
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    // Let browsers reuse the aggregate briefly. This endpoint fans out to
    // several upstream services, so even a small private cache window avoids
    // duplicate work during fast page navigation without serving stale war
    // data for long.
    res.setHeader(
      "Cache-Control",
